...
import { viewPendingApplications, savePropertyUpdateDetails, ViewSRTransactionDoc } from "../../services/assessment-services";
import { showToastError, showToastSuccess } from "../common/toastHelper";
import { getErrorMsg } from "../../utils/helpers";
import { srDashboardForZoSchema } from "../../utils/validation-schema";
import AlertMsg from "../common/alert";


const SRDashboardForZo = () => {
    ...
    const formik = useFormik({
        ...
        validationSchema: srDashboardForZoSchema,
        onSubmit: (values) => {
    ...
    useEffect(() => {
        ...
        const fetchPropertyDetails = async () => {
            try {
                setLoading(true);
                const response = await viewPendingApplications({
    ...
    const handleSubmit = async (actionType) => {
        ...
        try {
            ...
            if (response?.applicationId !== "") {
                ...
                setTimeout(() => navigate("/SRDashboardForZo"), 5000);
            } else {
    ...
    const handleDownload = async (documentId, documentName, documentURLbase64) => {
        try {
            const response = await ViewSRTransactionDoc(documentId, documentName, documentURLbase64);
    ...
    // Loading guard
    if (!responseData?.propertyTransactionVO?.length) return <div>Loading...</div>;
                            ...
                            {documents.map((doc, index) => (
                                <TableRow key={index}>
                                    ...
                                    <TableCell align="center">
                                        <VisibilityIcon
                                            fontSize="small"
                                            onClick={() => handleDownload(doc.documentId,doc.documentName, doc.documentURLbase64)}
                                            style={{
                                                color: "#1976d2",
                                                cursor: "pointer",
                                            }}
...
export default SRDashboardForZo;